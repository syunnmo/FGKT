...
class Model(nn.Module):
    def __init__(self, input_dim, hidden_dim, layer_dim, output_dim, num_of_question, num_of_problem, embed_d, num_heads,dropout, mode, params):
        super(Model, self).__init__()
        self.hidden_dim = hidden_dim
        self.n_question = num_of_question + 1
        self.n_pid = num_of_problem + 1
        ...
        self.rel = nn.ReLU()
        self.masked_attn_head_exercise = MultiHeadAttention_exercise(self.n_question, self.n_pid, embed_d, num_heads, mode, params.gpu)
    ...
    def forward(self, q_seq, qa_seq, pid_seq, target):
        ...
        index = qa_seq * qa_seq.size(-1) + torch.arange(0, qa_seq.size(-1), 1)
        ...
        location = qa_seq * qa_seq.size(-1) + torch.arange(0, qa_seq.size(-1), 1)
        ...
        h0 = Variable(torch.zeros(self.layer_dim, p_value.size(0), self.hidden_dim))
        c0 = Variable(torch.zeros(self.layer_dim, p_value.size(0), self.hidden_dim))
...
class MultiHeadAttention_exercise(nn.Module):
    def __init__(self, n_question, n_problem, embed_d, num_heads, mode, gpu):
        super().__init__()
        self.mode = mode
        self.gpu = gpu
        self.n_question = n_question
        self.n_problem = n_problem
        self.q_embed = nn.Embedding(n_question, embed_d, padding_idx=0)
        self.p_embed = nn.Embedding(n_problem, embed_d, padding_idx=0)
    ...
    def init_params(self):
        ...
        nn.init.kaiming_normal(self.p_embed.weight)
    ...
    def forward(self, q_seq, p_seq):
        q_embedd = self.q_embed(q_seq)
        p_embedd = self.p_embed(p_seq)

        if self.mode == 1:
            exercise_embedd = torch.cat((q_embedd, p_embedd), -1)
        elif self.mode == 2:
            p_embedd = self.E(p_embedd)
            exercise_embedd = q_embedd * p_embedd
        elif self.mode == 3:
            p_embedd = self.E(p_embedd)
            p_embedd = q_embedd * p_embedd
            exercise_embedd = torch.cat((q_embedd, p_embedd), -1)
        p_value = self.linear_q(exercise_embedd)
        p_query = self.linear_k(exercise_embedd)
        p_key = self.linear_v(exercise_embedd)
